        ...
        todos.map((todo) => (
            <div key={todo.id} className="flex mb-4 items-center">
                ...
                <div className="flex float-right ml-auto">
                    ...
                    <button className="flex-no-shrink p-2 ml-2 border-2 rounded text-red border-red-600 hover:text-white hover:bg-red-600" onClick={() => dispatch(deleteTodo(todo.id))}>Remove</button>
                </div>
            </div>
        ))
    )
